...
# usage:
#   python pdf_ocr_inv_to_json.py invoice_detail_report_20251003_72195.pdf --method auto
#   python pdf_ocr_inv_to_json.py invoice_detail_report_20251003_72195.pdf --method table
#   python pdf_ocr_inv_to_json.py invoice_detail_report_20251003_72195.pdf --method ocr
...
def transform_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'Invoice Date' in r and r['Invoice Date']:
        parsed = parse_date_to_iso(r['Invoice Date'])
        if parsed: r['Invoice Date'] = parsed
    ...
    if 'Invoice Received Date' in r and r['Invoice Received Date']:
        parsed = parse_datetime_to_iso(r['Invoice Received Date'])
        if parsed: r['Invoice Received Date'] = parsed
    ...
    # Related Document → remove "PO:" prefix
    if 'Related Document' in r and r['Related Document']:
        val = r['Related Document']
        r['Related Document'] = re.sub(r'^\s*PO:\s*', '', val, flags=re.IGNORECASE)
...
def main():
    ...
    parser.add_argument("filename", help="PDF file name inside raw_data/inv")
