...
class ImportPoReport extends Command
{
    protected $signature = 'po-report:import
        {path : Path to JSON file}
        {--table=po_detail_report : DB table name}
        {--connection= : DB connection name (override default)}
        {--pointer= : JSON pointer if root is object (e.g. /items). Leave empty for array root}
    ...
    protected $description = 'Insert-only importer for PO detail report (low-memory streaming, with diagnostics).';

    // นับจำนวนที่ถูกแก้กรณี YYYY-DD-MM -> YYYY-MM-DD สำหรับ send_date
    protected int $fixedSendDateSwap = 0;
    ...
    public function handle(): int
    {
        ...
        $pointer    = $this->option('pointer'); // null/'' => array root
        ...
        try {
            $items = ($pointer === null || $pointer === '')
                ...
                : Items::fromFile($path, ['pointer' => $pointer]);   // object root with pointer
        } catch (\Throwable $e) {
        ...
        $map = [
            ...
            'Send Date'               => 'send_date',               // DATETIME
            'Delivery Date'           => 'delivery_date',           // DATE
            'PO Received From Date'   => 'po_received_from_date',   // DATE
            'PO Received To Date'     => 'po_received_to_date',     // DATE
            'Amount (PO Include VAT)' => 'amount_incl_vat',
            'Status'                  => 'status',
        ];

        // กฎ validate หลัง normalize (คลาย send_date เป็น string เพราะเรา normalize เอง)
        $rules = [
            ...
            'send_date'             => 'nullable|string', // เราแปลงให้เป็น Y-m-d H:i:s แล้ว
            'delivery_date'         => 'nullable|date_format:Y-m-d',
            'po_received_from_date' => 'nullable|date_format:Y-m-d',
            'po_received_to_date'   => 'nullable|date_format:Y-m-d',
        ...
        foreach ($items as $raw) {
            ...
            // normalize: send_date = datetime (Y-m-d H:i:s) + auto-fix YYYY-DD-MM
            $row['send_date'] = $this->toYmdHisOrNull($row['send_date'] ?? null);

            // normalize: date-only fields
            foreach (['order_date', 'delivery_date', 'po_received_from_date', 'po_received_to_date'] as $dk) {
                $row[$dk] = $this->toYmdOrNull($row[$dk] ?? null);
            }

            // normalize: amount
            if (array_key_exists('amount_incl_vat', $row)) {
                $row['amount_incl_vat'] = $this->toNumberOrNull($row['amount_incl_vat']);
            }

            // จำกัดเฉพาะคอลัมน์ที่มีจริง และกัน array/object ออกจากค่า
            ...
            try {
                ...
                if (count($errorSamples) <= $maxLog) {
                    $this->output->writeln("");
                    $this->error("Row {$total} " . ($isDup ? 'duplicate_key' : 'db_error') . ": " . $e->getMessage());
                }
            } catch (\Throwable $e) {
                $failed++;
                $failDbOther++;
        ...
        $this->line("Fixed send_date (day-month swapped): {$this->fixedSendDateSwap}");
    ...
    /** แปลงเป็น Y-m-d H:i:s สำหรับ send_date; ตรวจจริง + auto-fix YYYY-DD-MM + ตัด token แปลก ๆ */
    protected function toYmdHisOrNull(?string $s): ?string
    {
        if ($s === null) return null;
        $s = trim((string) $s);
        if ($s === '' || $s === '-' || $s === '?' || strcasecmp($s, 'null') === 0 || strcasecmp($s, 'n/a') === 0) {
